...
if ~islogical(params.modulationProtectionEnabled) && ...
        ~(isnumeric(params.modulationProtectionEnabled) && isscalar(params.modulationProtectionEnabled))
    error('LatticeSIM:InvalidModulationProtectionFlag', ...
        'modulationProtectionEnabled must be a scalar logical or numeric value.');
end
if params.modulationMinReliable <= 0 || params.modulationMinReliable > 1
    error('LatticeSIM:InvalidModulationMinReliable', ...
        'modulationMinReliable must be in the range (0, 1].');
end
if params.modulationFallback <= 0 || params.modulationFallback > 1
    error('LatticeSIM:InvalidModulationFallback', ...
        'modulationFallback must be in the range (0, 1].');
end
...
if params.sidebandPairResidualWarningPixels < 0
    error('LatticeSIM:InvalidSidebandPairResidualWarningPixels', ...
        'sidebandPairResidualWarningPixels must be non-negative.');
end
...
if params.hifiOtfA <= 0 || params.hifiOtfA > 1
    error('LatticeSIM:InvalidHifiOtfA', ...
        'HiFi-style OTF damping factor must be in the range (0, 1].');
end
