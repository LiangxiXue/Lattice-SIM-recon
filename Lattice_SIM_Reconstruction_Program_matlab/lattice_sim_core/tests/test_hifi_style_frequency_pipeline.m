...
assert(strcmp(result.diagnostics.combine.fusionMode, 'hifi-style-direct-wiener'), ...
    'Expected Lattice fusion to use direct spectrum combination followed by Wiener filtering.');
...
assert(all(result.diagnostics.combine.finalConfidenceMask(result.diagnostics.combine.supportMask) == 1), ...
    'HiFi-style fusion should not apply a smooth confidence dip inside supported frequencies.');
