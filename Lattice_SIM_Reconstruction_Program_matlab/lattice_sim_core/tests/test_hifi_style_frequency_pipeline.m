...
expectedWidefield = imresize(mean(double(rawStack(:, :, 3:5)), 3), 2);
...
assert(strcmp(result.diagnostics.combine.fusionMode, 'single-step'), ...
    'Expected default Lattice fusion to preserve single-step Wiener-like filtering.');
assert(isfield(result.diagnostics.combine, 'fftDirectlyCombined'), ...
    'Expected direct combined spectrum diagnostics before Wiener filtering.');
assert(isfield(result.diagnostics.combine, 'wienerFilter'), ...
    'Expected Wiener filter diagnostics after direct combination.');
assert(isfield(result.diagnostics.combine, 'physicalOtfValues'), ...
    'Expected physical OTF diagnostics for the output canvas.');
assertSmoothConfidenceMask(result.diagnostics.combine);
assertSpectrumMatchesDirectWiener(result.diagnostics.combine);
...
assert(isequal(size(result.WF), size(expectedWidefield)), ...
    'Expected wide-field output to be 2x the raw frame size.');
assert(max(abs(result.WF(:) - expectedWidefield(:))) < 1e-10, ...
    'Expected wide-field output to be the 2x upsampled average of raw frames 3-5.');
assert(isSpatiallyAligned(result.SIM, result.WF), ...
    'Expected SIM output to stay aligned with the raw-frame-average wide-field image.');
...
function assertSpectrumMatchesDirectWiener(combineDiagnostics)
expectedSpectrum = combineDiagnostics.fftDirectlyCombined .* ...
    combineDiagnostics.wienerFilter .* ...
    combineDiagnostics.finalConfidenceMask .* ...
    combineDiagnostics.apodizationMask;
scale = max(1, max(abs(combineDiagnostics.simSpectrum(:))));
assert(max(abs(combineDiagnostics.simSpectrum(:) - expectedSpectrum(:))) / scale < 1e-12, ...
    'Final SIM spectrum should be direct-combined spectrum filtered by Wiener, support, and apodization.');
end

function assertSmoothConfidenceMask(combineDiagnostics)
transitionMask = combineDiagnostics.supportMask & ~combineDiagnostics.reliabilityMask;
if nnz(transitionMask) > 0
    transitionConfidence = combineDiagnostics.finalConfidenceMask(transitionMask);
...
assert(all(combineDiagnostics.finalConfidenceMask(~combineDiagnostics.supportMask) == 0), ...
    'Unsupported frequencies should have zero confidence.');
assert(all(combineDiagnostics.finalConfidenceMask(combineDiagnostics.reliabilityMask) == 1), ...
    'Reliable frequencies should have full confidence.');
end
